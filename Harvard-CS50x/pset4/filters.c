...
void grayscale(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < width; i++)
    {
        ...
        {
            // This line will average out the color intensity. It will apply the exact same value to all the colors to get gray
...
// Setting the value max 255 prevents bleeding on the edges and overflow
int limit(int RGB)
{
    if (RGB > 255)
    {
...
void reflect(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int j = 0; j < height; j++)
    {
        for (int i = 0; i < width / 2; i++)
        {
            ...
            // swapping pixels with the ones on the opposite side of the picture and viceversa
...
void blur(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < width; i++)
    {
        for (int j = 0; j < height; j++)
        {
            ...
            // sums values of the pixel and 8 neighboring ones, skips iteration if it goes outside the picture
            for (int k = -1; k < 2; k++)
            {
